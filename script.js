...
function obtenerCantidadTotal() { return carrito.reduce((acc, item) => acc + item.cantidad, 0); }
...
function agregarAlCarrito(nombre, precio, medida = "") {
  ...
  else { carrito.push({ nombre, precio, medida, cantidad: 1 }); }
  guardarCarrito(); renderCarrito(); actualizarContador(); animarCarrito();
}
...
function formatearPrecio(num) { return num.toLocaleString("es-AR"); }
function renderCarrito() {
  ...
  if (carrito.length === 0) {
    ...
    carrito.forEach((item, index) => {
      ...
      div.appendChild(info); div.appendChild(controles);
      div.appendChild(precio); div.appendChild(btnEliminar);
...
function initSelectorFechas() {
  ...
  fechaSalida.addEventListener("change", actualizarContadorDias);
  ...
  document.querySelectorAll(".btn-agregar").forEach((boton) => {
    boton.addEventListener("click", () => {
      ...
      if (contenedor.classList.contains("producto")) {
        ...
        if (selectMedida && !medida) { mostrarToast("Seleccioná una medida antes de agregar al carrito.", "warning"); return; }
      } else {
  ...
  if (listaEsquiadores) {
    ...
    function actualizarResumen() {
      ...
      esquiadores.forEach((esq, i) => {
        ...
        titulo.textContent = `${esq.nombre || `Esquiador ${i + 1}`} — ${esq.edad === "adulto" ? "Adulto" : "Niño"}${esq.tipo ? " · " + (
          esq.tipo === "ski" ? "Ski" : esq.tipo === "snow" ? "Snowboard" :
          esq.tipo === "solo_ski" ? "Solo Esquí" : esq.tipo === "solo_snow" ? "Solo Snowboard" :
          esq.tipo === "solo_bota_ski" ? "Bota Ski" : esq.tipo === "solo_bota_snow" ? "Bota Snowboard" : ""
      ...
      if (totalDiaEl) {
        const totalPorDia = esquiadores.reduce((acc, esq) => {
          ...
          if (esq.campera)         t += getPrecioLocal(esNino ? "campera_nino" : "campera_adulto", 1);
        ...
        try {
          ...
          if (data.ok) {
            ...
            localStorage.setItem("reservaConfirmada", JSON.stringify({
              numero, nombre: formData.get("nombre"), email: formData.get("email"),
              telefono: formData.get("telefono") || "", esquiadores: cantEsq, total: totalGeneral,
            }));
            localStorage.removeItem("fechaLlegada");
            localStorage.removeItem("fechaSalida");
            localStorage.removeItem("cantPersonas");
